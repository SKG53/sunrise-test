...
const NearYou = () => {
  ...
  return (
    <PageShell>
      ...
      <section className="site-section-tight"><div className="grid gap-5 md:grid-cols-[1.2fr_0.8fr]"><div className="map-zone min-h-[360px] bg-tier-30"><div><h2 className="text-3xl font-black uppercase tracking-[0.09em]">Retailer Footprint</h2><p className="mt-3 text-sm text-background/75">Map experience coming soon.</p></div></div><div className="rounded-md border border-border bg-card">{retailers.map((name,i)=><div key={name} className="border-b border-border p-5 last:border-b-0"><h3 className="text-sm font-black uppercase tracking-[0.12em]">{name}</h3><p className="mt-2 text-sm text-muted-foreground">{i + 1}.{i + 4} miles away · {filter === "All" ? "Retailer" : filter}</p></div>)}</div></div></section>
